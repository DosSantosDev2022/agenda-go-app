...
import { getInputsList } from "@/actions/inputs";
import { InputsPageData } from "@/types/inputs";
import { InfiniteData, useInfiniteQuery } from "@tanstack/react-query";

/**
 * @description Hook customizado para buscar insumos com scroll infinito.
...
export function useInfiniteInputsQuery(debouncedSearchTerm: string = "") {
  ...
  return useInfiniteQuery<
    ...
    ["inputsList", string], // TQueryKey: O tipo da Query Key
    InputCursor // TPageParam: O tipo do cursor (pageParam)
  >({
    queryKey: ["inputsList", debouncedSearchTerm],
